...
def test_consensus_success():
    ...
    return len(committed) >= 3  # quorum 3
...
def test_byzantine_primary_bad_hash():
    ...
    return not honest_bad  # must be True if safe
...
def test_byzantine_primary_equivocation():
    ...
    return not honest_commit_both  # safe if not both
...
def test_fault_limit_exceeded():
    ...
    return not honest_commit  # PASS nếu honest KHÔNG commit (liveness có thể mất nhưng safety giữ)
...
def test_persistency_no_duplicate_commit():
    ...
    return len(commits) >= 3  # consensus vẫn xảy ra nhưng không duplicate logic
